...
describe("IngestionPipeline", () => {
  ...
  it("ingests all events in order", () => {
    const events = [
      makeEvent("e-1", 1000),
      makeEvent("e-2", 2000),
      makeEvent("e-3", 3000),
    ...
    const ingested = pipeline.ingestAll();
    ...
    // Verify persisted
    expect(eventRepo.getAll()).toHaveLength(3);
  });
  ...
  it("deduplicates events by ID", () => {
    ...
    expect(eventRepo.getAll()).toHaveLength(2);
  });

  it("partial ingestion via ingestUpTo", () => {
    const events = [
      makeEvent("e-1", 1000),
      makeEvent("e-2", 2000),
      makeEvent("e-3", 5000),
    ...
    const batch1 = pipeline.ingestUpTo(3000);
    expect(batch1).toHaveLength(2);
    expect(pipeline.ingestedCount).toBe(2);
    expect(pipeline.done).toBe(false);

    // Ingest remaining
    const batch2 = pipeline.ingestUpTo(10000);
    expect(batch2).toHaveLength(2);
    expect(pipeline.ingestedCount).toBe(4);
    expect(pipeline.done).toBe(true);
  });

  it("notifies subscribers for each ingested event", () => {
  ...
  it("unsubscribe stops notifications", () => {
    ...
    pipeline.ingestUpTo(1500);
    unsub();
    pipeline.ingestUpTo(3000);
